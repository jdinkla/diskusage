/*
	Copyright (c) 2007, 2008, 2012 Joern Dinkla, www.dinkla.net, joern@dinkla.net
...
import net.dinkla.diskusage.TreeNode
...
class DirectoryForkJoin implements IDirectory {
    ...
    DirectoryForkJoin(String name) {
        ...
        withPool {
            traverse(root, directory, name)
        }
        root.reCalc()
        // sort the children by names
        root.each {
            it.children?.sort(TreeNode.sortNameAscending)
        }
    }
    ...
    */
    void traverse(TreeNode parent2, File directory2, String directoryName2) {
        runForkJoin(parent2, directory2, directoryName2) { TreeNode parent, File directory, String directoryName ->
            ...
            directory.eachFile { File file ->
                ...
                if (file.isDirectory()) {
                    ...
                    if (canon == absol) {
                        ...
                        TreeNode node = new TreeNode(name: parts[-1] + sep)
                        parent.add(node)
                        forkOffChild(node, file, fileNameOrig)
                    }
                } else {
                    // add the file
                    TreeNode node = new TreeNode(name: parts[-1], size: file.size())
                    parent.add(node)
                }
            }
        }
